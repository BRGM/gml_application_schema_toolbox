...
class TestModelBuilder(unittest.TestCase):
    ...
    def test1b(self):
        ...
        model = load_gml_model(os.path.join(testdata_path, "t1.xml"), "/tmp", merge_sequences = True)
    ...
    def test2(self):
        model = load_gml_model(os.path.join(testdata_path, "t2.xml"), "/tmp", merge_max_depth = 6, merge_sequences = False)
        self.assertEqual(len(model.tables()), 2)
        ta = set(["Column<d/text(),TEXT>",
                  "Link<c/cdetails/name(1-*),a_c_cdetails_name>",
                  "Column<c/type/text(),TEXT>",
        ...
        model = load_gml_model(os.path.join(testdata_path, "t2.xml"), "/tmp", merge_max_depth = 6, merge_sequences = True)
        self.assertEqual(len(model.tables()), 2)
        self.assertTableEqualRepr(model.tables()['a'], ta)
        self.assertTableEqualRepr(model.tables()['a_c_cdetails_name'], tb)        

    def test3(self):
        model = load_gml_model(os.path.join(testdata_path, "t3.xml"), "/tmp", merge_sequences = True)
        t = set(["Column<d/text(),TEXT>",
                 "Column<c/cdetails/name[0]/prefix/text(),TEXT,optional>",
                 "Column<c/type/text(),TEXT>",
                 "Column<b/text(),TEXT>",
    ...
    def test4(self):
        ...
        model = load_gml_model(os.path.join(testdata_path, "t4.xml"), "/tmp", merge_sequences = True)
    ...
    def xtest_geom1(self):
        ...
        model = load_gml_model(os.path.join(testdata_path, "t5.xml"), "/tmp", merge_sequences = True)
